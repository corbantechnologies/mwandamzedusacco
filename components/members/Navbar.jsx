...
function MemberNavbar() {
  ...
  const sidebarContent = (setIsMenuOpen) => (
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="p-6 border-b flex items-center justify-between">
        ...
        >
          <span className="text-xl font-bold tracking-tight text-[#045138]">
            Mwanda Mzedu
          ...
          return (
            <Link
              ...
              className={`block px-4 py-2.5 text-[14px] font-semibold rounded transition-colors ${
                isActive
                  ? "bg-[#045138]/10 text-[#045138]"
                  : "text-slate-600 hover:bg-slate-50 hover:text-[#045138]"
              }`}
              onClick={() => setIsMenuOpen && setIsMenuOpen(false)}
            >
              {link.label}
  ...
  return (
    <>
      {/* Top Navbar */}
      <header className="bg-[#045138] text-white sticky top-0 z-30 shadow h-16 flex items-center justify-between px-4 md:px-6 md:ml-64">
